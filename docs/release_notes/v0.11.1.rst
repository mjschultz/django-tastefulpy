...
1. Tastefulpy previously would accept a relation URI & *solely* parse out the
   ...
   would assign it to the ``User`` with a ``pk=1``. Tastefulpy
   would resolve the URI, but proceed to *only* care about the ``kwargs``, not
   validating it was for the correct resource.

   Tastefulpy now checks to ensure the resolving resource has a matching URI,
...
2. In some browsers (specifically Firefox), it was possible to construct a URL
   ...
   Tastefulpy now escapes all error messages that could be returned to the user
   to prevent this kind of attack in the future. Thanks to Micah Hausler for
   the report!

   Fixed in SHA: ae515bd

Should you find a security issue in Tastefulpy, please report it to
tastefulpy-security@googlegroups.com. Please **DO NOT** open GitHub issues or
post the issues on the main Tastefulpy mailing list. Thanks!
...
* Fixed the ``unquote`` import in ``tastefulpy.contrib.gis``. (SHA: 1958df0)
